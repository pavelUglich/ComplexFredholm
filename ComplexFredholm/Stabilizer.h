...
public:
	...
	Stabilizer(size_t n, double step, double p, BoundaryCondition Left = Neumann, BoundaryCondition Right = Neumann) :size(n) {
	...
	//~Stabilizer();
	vector<double> diagonal() { return Diagonal; }
	vector<double> Updiagonal() { return UpDiagonal; }
};
